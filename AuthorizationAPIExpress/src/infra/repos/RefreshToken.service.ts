...
export default class RefreshTokenService {
  ...
  public async VerifyToken(refreshToken: RefreshToken) {
    ...
    if (!refreshToken) {
      throw new AppError(HttpStatusCode.UNAUTHORIZED, "4400", "Refres token expired not exist", ErrorTypeEnum.TecnicalException);
    }

    /**IsExpired */
    if (refreshToken == null || this.IsExpired(refreshToken)) {
      const fe = new AppError(HttpStatusCode.UNAUTHORIZED, "4401", "Refresh token was expired or not exist", ErrorTypeEnum.TecnicalException);
      throw fe;
    }
  }
  ...
  public IsExpired(refreshToken: RefreshToken): boolean {
    ...
    // {
    //     console.log('La fecha almacenada en caché ha expirado');
    // } else {
    //   console.log("La fecha almacenada en caché aún no ha expirado");
    // }
    // const expired = Date.UTC >= refreshToken.Expires.getUTCDate;
    return expired;
  }
  ...
  public async GetStoredRefreshToken(tokenKey: string): Promise<RefreshToken> {
    return new Promise<RefreshToken>(async (resolve, reject) => {
      try {
        const rt = await this.cacheRepository.GetTk(tokenKey);
        if (!rt) throw new AppError(HttpStatusCode.UNAUTHORIZED, "4400", "Refres token expired not exist", ErrorTypeEnum.TecnicalException);
